...
6. **Speaker labels / diarization.** ✅ Shipped (6a + 6b, Windows + macOS). The
   sidecar backend is CI-proven end-to-end on BOTH windows-latest and macos-14
   (real diarization runs; the macOS universal2 static exe is self-contained).
   Only a full in-app run on a real multi-speaker video remains unexercised (needs
   a release build / the real sidecar locally). The heaviest item —
